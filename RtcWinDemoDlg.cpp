...
BOOL CRtcWinDemoDlg::OnInitDialog()
{
	...
	srand(GetTickCount64());  //提示将GetTickCount改为GetTickCount64()
	session_id_ = rand()%0xFFFF;
	m_editRoomId.SetWindowText(_T("bytertc"));
	m_editUserId.SetWindowText(_T("pc"));

	//初始化窗口分配队列
	video_queue_.push(IDC_REMOTE_WIN_1);
	video_queue_.push(IDC_REMOTE_WIN_2);
	video_queue_.push(IDC_REMOTE_WIN_3);
...
void CRtcWinDemoDlg::RemoveTrack(webrtc::VideoTrackInterface* track) {
	if (track && track->kind() == webrtc::MediaStreamTrackInterface::kVideoKind) {
		...
		auto qit = map_of_video_allocates_.find(track_id);
		if (qit != map_of_video_allocates_.end()) {
			video_queue_.push(qit->second);
			map_of_video_allocates_.erase(qit);
		}
	}
}
	...
	if (is_local) {
		...
		if (!video_queue_.empty()) {
			...
			if (pWnd) {
				...
				map_of_video_allocates_[track_id] = vid;
			}
		}
		else {
			AfxMessageBox(_T("No More Video Windows"));
			return NULL;
		}
	}
...
void CRtcWinDemoDlg::OnBnClickedBtnSend()
{
	if (joined_room_) {
		...
		m_editUserId.GetWindowText(UserName);
		if (chatMsg == "") {
			AfxMessageBox(_T("Cannot Send Blank Message"));
			return;
		}
		SendChatMessage(CStringToStdString(chatMsg));
		//消息发送成功己方聊天窗口也会显示发送信息
		UserName += ":\n";
		AppendMessage(UserName, true);
		chatMsg += "\n";
